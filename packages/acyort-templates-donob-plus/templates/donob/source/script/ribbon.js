...
const cos = Math.cos // cos函数返回一个数值的余弦值（-1~1）
const PI_2 = Math.PI * 2 // 圆周率*2

function generateColor(arg) {
  var color =
    '#' +
    (
      ((cos(arg) * 127 + 128) << 16) |
      ((cos(arg + PI_2 / 3) * 127 + 128) << 8) |
      (cos(arg + (PI_2 / 3) * 2) * 127 + 128)
    ).toString(16)
  return color
}
...
function ribbon() {
  ...
  function resize() {
    ...
    // RIBBON_WIDE = 90,
    ;(RIBBON_WIDE = (width / 10.5) * 0.75), (canvasRibbon.width = width * dpr) // 返回实际宽高
